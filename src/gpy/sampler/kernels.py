...
class Gaussian1DModel(ObservationModel):
    ...
    def like_existing(self, x: np.ndarray, atom: float) -> float:
        return norm.pdf(x, loc=atom, scale=self.sigma_x)

    def like_new_root(self, x: np.ndarray) -> float:
        return norm.pdf(x, loc=self.mu_phi, scale=np.sqrt(self.sigma_phi**2 + self.sigma_x**2))

    def sample_root_atom(self, x_obs: np.ndarray) -> float:
...
class Gaussian2DModel(ObservationModel):
    ...
    def like_existing(self, x: np.ndarray, atom: np.ndarray) -> float:
        return multivariate_normal.pdf(x, mean=np.asarray(atom), cov=self.Sigma_x)

    def like_new_root(self, x: np.ndarray) -> float:
        return multivariate_normal.pdf(x, mean=self.mu_phi, cov=self.Sigma_phi + self.Sigma_x)

    def sample_root_atom(self, x_obs: np.ndarray) -> np.ndarray:
